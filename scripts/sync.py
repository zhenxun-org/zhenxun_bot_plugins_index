...
def sync_repository(github_url, repo_name, auth_url):
    ...
    try:
        ...
        error_msg = "\n".join([line for line in error_lines if "error:" in line or "fatal:" in line])
        if not error_msg:
...
def main():
    ...
    # 添加所有必要的参数
    parser.add_argument('--changed-plugins', type=str, required=True, 
                        help='JSON格式的变更插件列表')
    parser.add_argument('--org-id', type=str, required=True, 
                        help='阿里云组织ID')
    parser.add_argument('--group-path', type=str, required=True, 
                        help='阿里云组路径')
    parser.add_argument('--aliyun-account', type=str, required=True, 
                        help='阿里云账号')
    parser.add_argument('--aliyun-password', type=str, required=True, 
                        help='阿里云密码')
    parser.add_argument('--access-key-id', type=str, required=True, 
                        help='阿里云AccessKey ID')
    parser.add_argument('--access-key-secret', type=str, required=True, 
