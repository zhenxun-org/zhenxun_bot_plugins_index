...
class Record(BaseMessageComponent):
    ...
    def __init__(self, file: T.Optional[str], **_):
        for k in _.keys():
            if k == "url":
                raise Exception("you should put url into file parameter")
                # raise Exception(f"go-cqhttp doesn't support send {self.type} by {k}")
    ...
    def __init__(self, file: str, **_):
        for k in _.keys():
            if k == "c" and _[k] not in [2, 3]:
                raise Exception(f"{k}={_[k]} doesn't match values")
        super().__init__(file=file, **_)
...
class Node(BaseMessageComponent):  # 该 component 仅支持使用 sendGroupForwardMessage 发送
    ...
    def toString(self):
        raise Exception("node doesn't support stringify")
        return ""
